...
class Fed_server(Process):
    ...
    def __init__(
        ...
        test_data, test_label,mutil_test_data,mutil_test_label, state_list=None, state_dict_list=None, idx=None
        ):
        ...
        self.writer = SummaryWriter(log_dir="runs/server")
        ...
        for client_i in range(len(per_client_label)):#全加起来了
        ...
        mutil_test_dataset = []
        for data,label in zip(mutil_test_data,mutil_test_label):
            mutil_test_dataset.append(test_dataset(data,label,config))
        self.mutil_test_dataset = mutil_test_dataset

    def evaluate_clients(self, selected_idx, round_i):  # 修改代码
        """
        Evaluate the selected clients' models on the test dataset and log the accuracy to TensorBoard.
        """
        self.server_network["feat_model"].eval()
        self.server_network["classifier"].eval()
        
        for i in selected_idx:
            network = self.networks[i]
            network["feat_model"].eval()
            network["classifier"].eval()

            test_loss_per_cls, test_acc_per_cls,_,_,_,_ = validate_one_model(
            network, self.test_dataset, self.device, per_cls_acc=True)
            ...
            #         output = network["classifier"](network["feat_model"](data))
            ...
            self.writer.add_scalar(f'client_{i}/accuracy', test_acc_mean, round_i)  # 修改代码


    def local_train(self, selected_idx, round_i):
        """
        server-side code
        """
        ...
        print(selected_idx)
        self.evaluate_clients(selected_idx,round_i)
    ...
    def evaluate_global(self, train_dataset=None, test_dataset=None):
        ...
        train_loss_per_cls, train_acc_per_cls,_,_,_,_= validate_one_model(
            self.server_network, train_dataset, self.device, per_cls_acc=True) 
        # evaluate on test set: per-class loss/acc
        test_loss_per_cls_ori, test_acc_per_cls_ori,_,_,_,_ = validate_one_model(
        ...
        results = {}  # 用于存储结果的字典
        for test_type, dataset in zip(test_distribution_set,self.mutil_test_dataset):
            _,_ ,head_acc, middle_acc,tail_acc, total_acc  = validate_one_model(
            self.server_network, dataset, self.device, per_cls_acc=True) 
            results[test_type] = {
            "head_acc": head_acc,
        ...
        return train_loss_per_cls, train_acc_per_cls, test_loss_per_cls_ori, test_acc_per_cls_ori, results
...
class Fed_client(Process):
    ...
    def __init__(
        self, init_network, criterion, config, per_client_data, 
        per_client_label, idx_per_client_train,
        test_data, test_label, global_num_per_class, state_list = None, state_dict_list=None,  idx=None
        ):
        ...
        self.status_list = state_list
        self.state_dict_list = state_dict_list
        self.global_num_per_class = global_num_per_class
        self.client_idx = idx   # physical idx of clients (hardcoded)
        ...
        self.client_weights = np.array([i for i in idx_per_client_train])
        self.client_weights = self.client_weights/self.client_weights.sum()#每个客户端的权重
        ...
        self.losses_cls = [-1 for i in range(self.num_clients)]
        self.losses_kd = [-1 for i in range(self.num_clients)]
        self.cos_similarity_value_per_client = np.zeros(self.num_clients,dtype=float)
        self.writer = None#新增代码
        ...
        for client_i in range(self.num_clients):
            ...
            #print("第{}个客户端".format(client_i))
            cos_similarity_value, weight_sampler = local_dataset.get_balanced_sampler(global_num_per_class) #修改
            self.cos_similarity_value_per_client[client_i] = cos_similarity_value
            self.train_loader_balanced.append(
                torch.utils.data.DataLoader(
                   local_dataset, batch_size=self.local_bs, sampler = weight_sampler,#sampler=local_dataset.get_balanced_sampler(global_num_per_class), #修改
                   num_workers=num_workers, pin_memory=False)
            )
            self.local_num_per_cls.append(local_dataset.class_sample_count)
    ...
    def run(self):
        ...
        client-side code
        """
        self.writer = SummaryWriter(log_dir=f'logs/client_{self.client_idx}') #新增代码
        self.server_network["feat_model"].to(self.device)
        self.server_network["classifier"].to(self.device)
...
#   def evaluate_and_log(self, round_i):  # 新增代码
#         """
#         Evaluate the model on the test dataset and log the accuracy to TensorBoard
#         """

#         loss, acc_per_cls = validate_one_model(self.networks[self.client_idx], self.test_dataset, self.device, per_cls_acc=True)  # 新增代码
#         avg_acc = sum(acc_per_cls) / len(acc_per_cls)  # 新增代码
#         print(f"Client {self.client_idx} - Round {round_i}: Accuracy {avg_acc}")  # 新增代码
#         self.writer.add_scalar('Accuracy', avg_acc, round_i)  # 新增代码
    ...
    def train_lt(self, idx):   
        ...
        """ 
        idx_in_all = idx
        self.per_client_acc = np.array(self.num_clients, dtype=float)
        ...
        with torch.set_grad_enabled(True):  
            ...
            for epoch in range(self.local_ep):   
                ...
                for (imgs, labels, indexs) in tmp_loader:
                    ...
                    #writer = SummaryWriter(log_dir='./train_per_client')
                    # Collecting predictions and labels
                    preds = torch.argmax(logits, dim=1).cpu().numpy()  # 新增代码
                    all_preds.extend(preds)  # 新增代码
                    all_labels.extend(labels.cpu().numpy())  # 新增代码
                ...
                all_labels = np.array(all_labels).squeeze()  # 新增代码
                all_preds = np.array(all_preds).squeeze()  # 新增代码
                unique_labels_eva = np.unique(all_labels)  # 新增代码
                accuracies = {}  # 新增代码
                for label in unique_labels_eva:  # 新增代码
                    label_mask = all_labels == label  # 新增代码
                    accuracies[label] = accuracy_score(all_labels[label_mask], all_preds[label_mask])  # 新增代码
                avg_accuracy = accuracy_score(all_labels, all_preds)  # 新增代码

            print(f"第{idx}个客户端：  average accuracy: {avg_accuracy}")  # 新增代码
