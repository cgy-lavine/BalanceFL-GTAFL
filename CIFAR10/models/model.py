...
class BBN_ResNet_PathMNIST(BBN_ResNet_Cifar):
    def __init__(self, block, num_blocks, l2_norm, feat_dim=128):
        super(BBN_ResNet_PathMNIST, self).__init__(block, num_blocks, l2_norm, feat_dim)
        ...
        self.layer1 = self._make_layer(block, channels[0], num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, channels[1], num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, channels[2], num_blocks[2] - 1, stride=2)
...
class BBN_ResNet_TissueMNIST(BBN_ResNet_Cifar):
    def __init__(self, block, num_blocks, l2_norm, feat_dim=128):
        super(BBN_ResNet_TissueMNIST, self).__init__(block, num_blocks, l2_norm, feat_dim)
        ...
        # 修改第一层卷积，适应1通道输入（TissueMNIST是灰度图像）
    ...
    def load_model(self, pretrain):
        ...
        for k, v in pretrain_dict.items():
            if k.startswith("module"):
                k = k[7:]
            if "fc" not in k and "classifier" not in k:
                # 特殊处理第一层卷积层：从3通道转换为1通道
                if k == "conv1.weight":
                    if v.size(1) == 3:  # 预训练模型是3通道
                        # 方法1：取RGB三个通道的平均值
                        v = torch.mean(v, dim=1, keepdim=True)
                        # 或者方法2：使用加权平均
                        # v = 0.299 * v[:, 0:1, :, :] + 0.587 * v[:, 1:2, :, :] + 0.114 * v[:, 2:3, :, :]
                new_dict[k] = v
        
        # 更新模型参数
        missing_keys, unexpected_keys = model_dict.update(new_dict)
        self.load_state_dict(model_dict)
