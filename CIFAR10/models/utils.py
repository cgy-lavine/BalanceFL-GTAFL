...
def init_models(config):
    ...
    """
    from models.model import BBN_ResNet_Cifar, BasicBlock, DotProduct_Classifier, ResNet, Bottleneck, BBN_ResNet_PathMNIST, BBN_ResNet_TissueMNIST
    ...
    if config["dataset"]["name"] in ["CIFAR10", "CIFAR100"]:
        if "ResNet32" in config["networks"]["feat_model"]["def_file"]: 
            # feat_dim = 512
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = BBN_ResNet_Cifar(BasicBlock, [5,5,5], l2_norm, feat_dim).to(device)
        elif "ResNet20" in config["networks"]["feat_model"]["def_file"]: 
            # feat_dim = 256
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = BBN_ResNet_Cifar(BasicBlock, [3,3,3], l2_norm, feat_dim).to(device)
        elif "ResNet8" in config["networks"]["feat_model"]["def_file"]: 
            # feat_dim = 128
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = BBN_ResNet_Cifar(BasicBlock, [1,1,1], l2_norm, feat_dim).to(device)
        else:
            raise NotImplementedError
    elif config["dataset"]["name"] == "PathMnist":
        if "ResNet8" in config["networks"]["feat_model"]["def_file"]:
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = BBN_ResNet_PathMNIST(BasicBlock, [1,1,1], l2_norm, feat_dim).to(device)
        else:
            raise NotImplementedError
        ...
        if "ResNet8" in config["networks"]["feat_model"]["def_file"]:
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            print("TissueMnist")
            network["feat_model"] = BBN_ResNet_TissueMNIST(BasicBlock, [1,1,1], l2_norm, feat_dim).to(device)
        else:
            raise NotImplementedError
