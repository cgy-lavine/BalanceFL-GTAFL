...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', default="config/local.yaml", type=str)
    ...
    # config
    log_dir = f'{args.work_dir}/{args.tao_ratio}_{args.exp_name}'  
    ...
    for i in range(num_client):  
        p = Fed_client(
            network, criterion, config, per_client_data, per_client_label, training_num_per_cls, 
            test_data, test_label, state_list, state_dict_list, idx=i
            ) 
        p.daemon = True
        p.start()
    ...
    if not args.test:
        ...
        for round_i in range(1):
            ...
            all_results = fed.evaluate_global_all()
