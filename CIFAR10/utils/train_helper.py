...
def acc_loss_per_cls(loss_all, correct_all, labels_all, cls_num):
    ...
    first_three_correct = sum([sum(acc_record_per_cls[i]) for i in range(min(3, cls_num))])
    first_three_total = sum([len(acc_record_per_cls[i]) for i in range(min(3, cls_num))])
    middle_three_correct = sum([sum(acc_record_per_cls[i]) for i in range(3, min(6, cls_num))])
    middle_three_total = sum([len(acc_record_per_cls[i])for i in range(3, min(6, cls_num))])
    last_four_correct = sum([sum(acc_record_per_cls[i]) for i in range(max(0, cls_num - 4), cls_num)])
    last_four_total = sum([len(acc_record_per_cls[i]) for i in range(max(0, cls_num - 4), cls_num)])


    first_three_acc = first_three_correct / first_three_total if first_three_total > 0 else 0
    middle_three_acc = middle_three_correct / middle_three_total if middle_three_total > 0 else 0
    last_four_acc = last_four_correct / last_four_total if last_four_total > 0 else 0

    # Calculate total accuracy
    total_correct = sum([sum(acc_record_per_cls[i]) for i in range(cls_num)])
    total_samples = sum([len(acc_record_per_cls[i]) for i in range(cls_num)])
    total_acc = total_correct / total_samples if total_samples > 0 else 0


    return loss_per_cls, acc_per_cls,first_three_acc, middle_three_acc, last_four_acc, total_acc
...
def validate_one_model(model, dataset, device, per_cls_acc=False):
    ...
    if per_cls_acc is True:
        ...
        with torch.no_grad():
            ...
            loss_per_cls, acc_per_cls,first_three_acc, middle_three_acc, last_four_acc, total_acc = acc_loss_per_cls(loss_all, correct_all, labels_all, cls_num)

        return (np.array(loss_per_cls), np.array(acc_per_cls),first_three_acc, middle_three_acc, last_four_acc, total_acc)
